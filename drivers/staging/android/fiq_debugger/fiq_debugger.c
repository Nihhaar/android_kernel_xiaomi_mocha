			       ...
			       const char *fmt, ...)
{
	...
	if (NULL != state->pstore_console)
		state->pstore_console->write(state->pstore_console, buf, n);
	fiq_debugger_puts(state, buf);
}
		...
		const struct pt_regs *regs, void *svc_sp)
{
	...
	if (this_cpu == 0) {
		...
		do {
			need_irq = fiq_debug_handle_uart_interrupt(state, this_cpu, regs, svc_sp);
		} while (!need_irq);
		fiq_debug_force_irq(state);
#else
		need_irq = fiq_debug_handle_uart_interrupt(state, this_cpu, regs, svc_sp);
		if (need_irq)
			fiq_debug_force_irq(state);
